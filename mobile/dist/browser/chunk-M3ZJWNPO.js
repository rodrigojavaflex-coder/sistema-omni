...
var VistoriaService = class _VistoriaService {
  ...
  baixarPdfPendenciasVeiculo(idVeiculo, filtros) {
    return __async(this, null, function* () {
      return firstValueFrom(this.http.get(this.montarUrlPdfPendenciasVeiculo(idVeiculo, filtros), {
        responseType: "blob"
      }));
    });
  }
  montarUrlPdfPendenciasVeiculo(idVeiculo, filtros) {
    ...
    return `${this.apiBaseUrl}/vistoria/veiculo/${idVeiculo}/historico-irregularidades-nao-resolvidas/pdf${query ? `?${query}` : ""}`;
  }
  ...
  listarIrregularidadesImagens(vistoriaId, irregularidadeId) {
    return __async(this, null, function* () {
      const query = irregularidadeId ? `?irregularidadeId=${encodeURIComponent(irregularidadeId)}` : "";
      return firstValueFrom(this.http.get(`${this.apiBaseUrl}/vistoria/${vistoriaId}/irregularidades/imagens${query}`));
    });
  }
  listarIrregularidadesAudios(vistoriaId, irregularidadeId) {
    return __async(this, null, function* () {
      const query = irregularidadeId ? `?irregularidadeId=${encodeURIComponent(irregularidadeId)}` : "";
      return firstValueFrom(this.http.get(`${this.apiBaseUrl}/vistoria/${vistoriaId}/irregularidades/audios${query}`));
    });
  }
  ...
  removerAudiosIrregularidade(irregularidadeId) {
    return __async(this, null, function* () {
      yield firstValueFrom(this.http.delete(`${this.apiBaseUrl}/irregularidades/${irregularidadeId}/audio`));
    });
  }
...
//# sourceMappingURL=chunk-M3ZJWNPO.js.map
